...
- 后台预取失败保持静默，不能消耗用户的一次“加载更多”。用户明确点击后必须重新发起一次前台分页请求；若该请求仍失败，再显示真实错误并保留已有卡片与继续加载入口，不做自动循环重试。
...
- 来源导航始终位于工作区上方，让画廊使用完整宽度；X/Web/图库、Imagine、个人相册/本地库各自收进紧凑的分段组，并利用可用横向空间分开呈现，不增加组标题、解释段落、分组外框或嵌套卡片。
- 带文字的来源标签可横向滚动，不能退回只有图标、无法辨认来源的等宽七宫格；切换来源后当前标签应滚入可见区域。
